...
from backend.app.services.transcription import Speech2Text
...
class TestSpeech2Text:
    ...
    @pytest.mark.unit
    def test_prepare_audio_file(self, speech2text: Any, filename = "nonexistent.mp3"):
    ...
    def test_prepare_audio_success(self, mock_librosa, speech2text: Any, sample_audio_file: Any, mock_audio_data: tuple[NDArray[float64], Literal[16000]]):
    ...
    @patch('src.audio_processing.speech2text.librosa.load') #patch sieht was es returnen soll
    def test_transcribe_audio(self, mock_librosa, mock_no_grad, speech2text: Any, sample_audio_file: Any, mock_audio_data: tuple[NDArray[float64], Literal[16000]]):
